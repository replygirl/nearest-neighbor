...
<!-- No existing spec covers these five write routes (openspec/specs/ holds no
     content-moderation capability), so the entire surface is documented here as
     ADDED requirements rather than as a delta against a non-existent base —
     same precedent as the archived post-likes-and-reposts change. -->
