...
# ── Wrapper writer ──────────────────────────────────────────────────────────────
# Writes the portable nbr wrapper to <path>. The wrapper derives NBR_CONFIG_DIR
# from its own location and defaults NBR_NO_KEYRING, so config isolation is in
# effect regardless of which host (Claude Code, Codex, or any other agent)
# invokes it — no env-file mechanism required.
#
...
write_nbr_wrapper() {
  wrapper_target="$1"
  # SC2016: the wrapper file must contain literal ${SELF_DIR}/${NBR_*} — these
  # are expanded by the wrapper at run time, not by this installer.
  # shellcheck disable=SC2016
  {
    echo '#!/bin/sh'
    echo '# nbr wrapper — portable config dir derived from this script location.'
    echo '# DO NOT EDIT — regenerated by install-nbr.sh on each install.'
    echo 'SELF_DIR=$(cd "$(dirname "$0")" && pwd)'
    echo 'export NBR_CONFIG_DIR="${NBR_CONFIG_DIR:-${SELF_DIR}/../config/nbr}"'
    echo 'export NBR_NO_KEYRING="${NBR_NO_KEYRING:-1}"'
    if [ -n "${NBR_WRAPPER_API_URL:-}" ]; then
      printf 'export NBR_API_URL="%s"\n' "${NBR_WRAPPER_API_URL}"
    fi
    echo 'exec "${SELF_DIR}/.nbr-real" "$@"'
  } > "$wrapper_target"
