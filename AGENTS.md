...
OpenCode is the secondary agent runtime, preconfigured for local inference.
Config lives in `.opencode/opencode.json`. Skills are loaded from
`.claude/skills/` (OpenCode reads Claude Code skill paths natively) — no
duplication needed.

Model: `qwen3/qwen3.6-35b-a3b-fp8` via LMStudio at `http://127.0.0.1:1234/v1`.
Start LMStudio and load `qwen3.6-35b-a3b-fp8` before using OpenCode. If the
model fails to load, verify the exact model ID via
`curl http://127.0.0.1:1234/v1/models`.

Built-in agents available: `Build` (default), `Plan` (no-edit, for planning),
`Explore` (read-only). Use these directly rather than creating duplicates.

Plugin events (`tool.execute.after` for edit/write) trigger oxfmt on changed
files for in-session feedback. hk still enforces at commit time — do not disable
either layer.

## Cursor notes

Cursor is the backstop editor for contributors who prefer it. Config lives in
`.cursor/`. Rules in `.cursor/rules/` supplement this `AGENTS.md` with
path-scoped guidance (Elysia patterns, testing conventions, etc.).

MCP servers are declared in `.cursor/mcp.json`. Use `${env:X}` interpolation for
secrets — never hard-code tokens.

Hooks in `.cursor/hooks.json` run `oxfmt` after file edits and guard against
destructive shell patterns. These are edit-time; hk handles commit-time
enforcement. Do not disable either layer.
